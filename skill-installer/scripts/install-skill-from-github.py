#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into $ANTIGRAVITY_HOME/skills."""
...
def _antigravity_home() -> str:
    return os.environ.get("ANTIGRAVITY_HOME", os.path.expanduser("~/.gemini/antigravity"))


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "antigravity")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "antigravity-skill-install")
...
def _find_workspace_root() -> str | None:
    curr = os.getcwd()
    while True:
        if os.path.isdir(os.path.join(curr, ".agent")):
            return curr
...
def _default_dest(is_global: bool) -> str:
    if is_global:
        return os.path.join(_antigravity_home(), "skills")
    
    ws_root = _find_workspace_root()
    if ws_root:
        return os.path.join(ws_root, ".agent", "skills")
    
    # Fallback to global if no workspace found, but maybe we should warn?
    # User said "Default install in Workspace", so if not in workspace, let's error or fallback.
    return os.path.join(_antigravity_home(), "skills")
...
def _parse_args(argv: list[str]) -> Args:
    ...
    parser.add_argument("--global", dest="is_global", action="store_true", help="Install globally")
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        dest_root = args.dest or _default_dest(args.is_global)
