...
$ErrorLogFile = Join-Path (Join-Path $CtiHome 'logs') 'bridge.err.log'
...
function Test-PidAlive {
    param([string]$BridgeProcessId)
    if (-not $BridgeProcessId) { return $false }
    try { $null = Get-Process -Id ([int]$BridgeProcessId) -ErrorAction Stop; return $true }
    catch { return $false }
}
