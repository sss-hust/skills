...
**内容**：{配图内容}
"@
$prompt | Out-File -FilePath "$env:TEMP\image-prompt.txt" -Encoding UTF8

# Step 2: 调用脚本
npx -y bun "C:\Users\aoeiu\.gemini\antigravity\skills\smart-illustrator\scripts\generate-image.ts" `
  --prompt-file "$env:TEMP\image-prompt.txt" `
  --output "{输出路径}.png" `
  --aspect-ratio 3:4
```
...
- 视觉隐喻：{设计}
"@
$prompt | Out-File -FilePath "$env:TEMP\cover-prompt.txt" -Encoding UTF8

# Step 2: 调用脚本
npx -y bun "C:\Users\aoeiu\.gemini\antigravity\skills\smart-illustrator\scripts\generate-image.ts" `
  --prompt-file "$env:TEMP\cover-prompt.txt" `
...
# 执行方式
# 执行方式
Get-Content -Path "生成的JSON文件" -Raw -Encoding UTF8 | Set-Clipboard
Write-Host "✓ JSON prompt 已复制到剪贴板"

# 同时保存备份
'{生成的 JSON}' | Out-File -FilePath "$env:TEMP\smart-illustrator-prompt.json" -Encoding UTF8
Write-Host "✓ 备份已保存到 $env:TEMP\smart-illustrator-prompt.json"
```
